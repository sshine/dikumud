...
void clone_char(struct char_data *ch)
{
	...
	GET_NAME(clone) = our_strdup(GET_NAME(ch));

	clone->player.short_descr =	our_strdup(ch->player.short_descr);

	clone->player.long_descr = our_strdup(ch->player.long_descr);

	clone->player.description = 0;
	/* REMEMBER EXTRA DESCRIPTIONS */

	GET_TITLE(clone) = our_strdup(GET_TITLE(ch));
...
void clone_obj(struct obj_data *obj)
{
	...
	*clone = *obj;
	...
	clone->name               = our_strdup(obj->name);
	clone->description        = our_strdup(obj->description);
	clone->short_description  = our_strdup(obj->short_description);
	clone->action_description = our_strdup(obj->action_description);
